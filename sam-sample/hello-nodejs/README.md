...
```bash
hello-sam-nodejs20$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello-sam-nodejs20$ sam local start-api
hello-sam-nodejs20$ curl http://localhost:3000/
```
...
```bash
hello-sam-nodejs20$ sam logs -n HelloWorldFunction --stack-name hello-sam-nodejs20 --tail
```
...
```bash
hello-sam-nodejs20$ cd hello-world
...
```bash
sam delete --stack-name hello-sam-nodejs20
```
